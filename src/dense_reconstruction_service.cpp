...
#include "stereo_dense_reconstruction/StereoReconstructionSrv.h"
#include <fstream>
#include <ctime>
...
void CalculatePointCloud(Mat& img_left, Mat& dmap, 
...
//   pc.header.stamp = ros::Time::now();
  ...
  // Prepares Pointcloud2
  int max_point_num = img_left.cols * img_left.rows;
  pointcloud2->header.frame_id = "jackal";
  pointcloud2->width  = 0;
...
//   modifier.resize(max_point_num);
  ...
  for (int i = 0; i < img_left.cols; i++) {
    for (int j = 0; j < img_left.rows; j++) {
      ...
      // top_j: 150, bottom_j: 30
      if (j < 150 || j > (img_left.rows - 30)) {
        continue;
      }
      
      // i_margin for both sides 100 pixels: 
      if (i < 100 || i > (img_left.cols - 100)) {
        continue;
      }
...
void imgCallback(const sensor_msgs::Image& msg_left, 
                 const sensor_msgs::Image& msg_right,
                 sensor_msgs::PointCloud2* pointcloud2) {
  
  sensor_msgs::Image::Ptr msg_left_ptr = 
                              boost::make_shared<sensor_msgs::Image>();
  msg_left_ptr = boost::make_shared<sensor_msgs::Image>(msg_left);
  
  sensor_msgs::Image::Ptr msg_right_ptr = 
                              boost::make_shared<sensor_msgs::Image>();
  msg_right_ptr = boost::make_shared<sensor_msgs::Image>(msg_right);
  
  Mat tmpL = cv_bridge::toCvShare(msg_left_ptr, "mono8")->image;
  Mat tmpR = cv_bridge::toCvShare(msg_right_ptr, "mono8")->image;
  if (tmpL.empty() || tmpR.empty())
    return;
  ...
  Mat dmap = generateDisparityMap(img_left, img_right);
  CalculatePointCloud(img_left_color, dmap, pointcloud2);
...
bool StereoReconstructionSrv(
  stereo_dense_reconstruction::StereoReconstructionSrv::Request& req,
  stereo_dense_reconstruction::StereoReconstructionSrv::Response& res) {
  imgCallback(req.left_image, req.right_image, &res.point_cloud);
...
int main(int argc, char** argv) {
  ...
  int service_topic_num;
  
  static struct poptOption options[] = {
    { "left_topic",'l',POPT_ARG_STRING,&left_img_topic,0,
      "Left image topic name","STR" },
    { "right_topic",'r',POPT_ARG_STRING,&right_img_topic,0,
      "Right image topic name","STR" },
    { "calib_file",'c',POPT_ARG_STRING,&calib_file_name,0,
      "Stereo calibration file name","STR" },
    { "calib_width",'w',POPT_ARG_INT,&calib_width,0,
      "Calibration image width","NUM" },
    { "calib_height",'h',POPT_ARG_INT,&calib_height,0,
      "Calibration image height","NUM" },
    { "out_width",'u',POPT_ARG_INT,&out_width,0,"Rectified image width","NUM" },
    { "out_height",'v',POPT_ARG_INT,&out_height,0,
      "Rectified image height","NUM"},
    { "debug",'d',POPT_ARG_INT,&debug,0,
      "Set d=1 for cam to robot frame calibration","NUM" },
    { "service_topic_num",'n',POPT_ARG_INT,&service_topic_num,0,
      "Set the number that will be appended to the service topic name"
      ,"NUM" },
    POPT_AUTOHELP
...
//   sync.registerCallback(boost::bind(&imgCallback, _1, _2));
  ...
  std::string node_name = "jpp_dense_reconstruction_";
  char node_name_suffix [10];
  sprintf(node_name_suffix, "%d", service_topic_num);
  node_name.append(node_name_suffix);
  
  ros::init(argc, argv, node_name.c_str());
  ros::NodeHandle nh;
  image_transport::ImageTransport it(nh);
  
  std::string service_topic_name = "/stereo/dense_reconstruction_";
  char out_string [10];
  sprintf(out_string, "%d", service_topic_num);
  service_topic_name.append(out_string);
  ros::ServiceServer stereo_reconstruction_srv = nh.advertiseService(
      service_topic_name.c_str(), StereoReconstructionSrv);
  
  std::cout << "Stereo dense reconstruction service listening to requests on "
            << service_topic_name << std::endl;
